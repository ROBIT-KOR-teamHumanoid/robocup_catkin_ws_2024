...
void MainWindow::RxCallBack()
{
    ...
    for(int i = 0; i < a.size(); i++){
        //        rx_buffer[rx_idx++] = unsigned(a[i] & 0xff);
        if((a[i]&0xff) == 0xff){
            cout<<"MotionEND"<<endl;
    ...
    while (token != NULL) {
        ...
        rx_Data.push_back(token);
        token = strtok(NULL,"\n");
    }
    for(int i = 0; i< rx_Data.size(); i++){
//        cout<<"i == "<<i<<"\\\ "<<rx_Data.data()[i]<<endl<<strlen(rx_Data.data()[i])<<endl<<endl;

        char *tmpSTR = strstr(rx_Data.data()[i], "LOADCELL");
...
//                cout<<strlen(token)<<endl;
    ...
    if(lc_Data.size() == 10){
        ...
        loadcell_info.R_LC_Data.push_back(lc_Data.data()[1]);
        loadcell_info.R_LC_Data.push_back(lc_Data.data()[2]);
        loadcell_info.R_LC_Data.push_back(lc_Data.data()[3]);
        loadcell_info.R_LC_Data.push_back(lc_Data.data()[4]);

        loadcell_info.L_LC_Data.push_back(lc_Data.data()[5]);
        loadcell_info.L_LC_Data.push_back(lc_Data.data()[6]);
        loadcell_info.L_LC_Data.push_back(lc_Data.data()[7]);
        loadcell_info.L_LC_Data.push_back(lc_Data.data()[8]);

        qnode.loadcell_pub.publish(loadcell_info);
        loadcell_info.R_LC_Data.clear();
        loadcell_info.L_LC_Data.clear();
                        ...
                        unsigned char ParameterLength, unsigned char Motion_Mode, unsigned char MotionNum)
{
    ...
    else
    {
        ...
        //        m_serial -> write(Tx_data,PacketLength);
        Tx_data.clear();
