...
#include <iostream>
#include <fstream>
#include "../include/robocup_localization24_obstacle/main_window.hpp"
...
extern ros::Subscriber techobsSub;
extern ros::Subscriber techobslocalSub;

extern ros::Publisher localizationPub;
msg_generate::robocup_localization24_obstacle localizationMsg;
...
//technical obstacle navigation
int obs_sector = 0;
int obs_obs1 = 0;
int obs_obs2 = 0;

vector<int> obslocal_theta1;
vector<int> obslocal_theta2;
vector<int> obslocal_dist1;
vector<int> obslocal_dist2;
    ...
    , qnode(argc,argv)
{
    ...
    QString img_path = "/home/robit/catkin_ws/src/robocup_localization24_obstacle/resources/Map/2022_RoboCup_Field.png";
...
//    robot0.x = 550; robot0.y = 400; //로봇 시작 포인트, 맵의 중앙
    robot0.x = 625; robot0.y = 790; //로봇 시작 포인트, 맵의 중앙
...
** Functions
*****************************************************************************/
void robocup_localization24_obstacle::MainWindow::main()
{
    //INIT
    //        robot1.x = 550;
    //        robot1.y = 400;
...
void robocup_localization24_obstacle::MainWindow::sel_ball()
{
    ...
    //다른 로봇에서 공 데이터가 존재할 시 현재 로봇과 거리 측정, 존재하지 않을 시 비활성화
    if(ball1.d != 999999)
    {
        ball_err[0] = sqrt(pow(ball1.x - ball.x, 2) + pow(ball1.y - ball.y, 2));
        ball_cnt++;
        robot_num = 0;
    }
    else
    ...
    if(ball2.d != 999999)
    {
        ball_err[1] = sqrt(pow(ball2.x - ball.x, 2) + pow(ball2.y - ball.y, 2));
        ball_cnt++;
        robot_num = 1;
    }
    else
    ...
    if(ball3.d != 999999)
    {
        ball_err[2] = sqrt(pow(ball3.x - ball.x, 2) + pow(ball3.y - ball.y, 2));
        ball_cnt++;
        robot_num = 2;
    }
    else
    ...
    //공의 거리 오차 총합 후 값이 존재 할 시 공 위치 추정
    int ball_err_ = 0;
    int ballThreshold = 30;

    for(int i = 0 ; i < 4 ; i++) {ball_err_ += ball_err[i];}

    if(true){
        if(ball_cnt > 1)
            //공의 데이터가 2개 이상 존재 할 시
        {
            if(ball_err_ != 0)
                //공의 데이터가 1개 존재 할 시 2
            {
                ball_err_ /= ball_cnt;
                if(ball_err_ > ballThreshold)
                    //거리의 오차가 임계값 이상일 경우
                {
                    //각 로봇과 다른 로봇의 거리 오차를 로봇마다 계산
                    int ball_err_combi[6] = {0, };
                    ball_err_combi[0] = sqrt(pow(ball1.x - ball2.x, 2) + pow(ball1.y - ball2.y, 2));
                    ball_err_combi[1] = sqrt(pow(ball1.x - ball3.x, 2) + pow(ball1.y - ball3.y, 2));
                    ball_err_combi[2] = sqrt(pow(ball1.x - ball.x, 2) + pow(ball1.y - ball.y, 2));
                    ball_err_combi[3] = sqrt(pow(ball2.x - ball3.x, 2) + pow(ball2.y - ball3.y, 2));
                    ball_err_combi[4] = sqrt(pow(ball2.x - ball.x, 2) + pow(ball2.y - ball.y, 2));
                    ball_err_combi[5] = sqrt(pow(ball3.x - ball.x, 2) + pow(ball3.y - ball.y, 2));

                    int ball_err_robot[4] = {0, };

                    //로봇의 공 데이터가 존재하지 않을 경우 예외처리
                    if(ball_err[0] == 0){ball_err_combi[0] = 9999; ball_err_combi[1] = 9999; ball_err_combi[2] = 9999;}
                    if(ball_err[1] == 0){ball_err_combi[0] = 9999; ball_err_combi[3] = 9999; ball_err_combi[4] = 9999;}
                    if(ball_err[2] == 0){ball_err_combi[1] = 9999; ball_err_combi[3] = 9999; ball_err_combi[5] = 9999;}
                    if(ball_err[3] == 0){ball_err_combi[2] = 9999; ball_err_combi[4] = 9999; ball_err_combi[5] = 9999;}

                    ball_err_robot[0] = ball_err_combi[0] + ball_err_combi[1] + ball_err_combi[2];
                    ball_err_robot[1] = ball_err_combi[0] + ball_err_combi[3] + ball_err_combi[4];
                    ball_err_robot[2] = ball_err_combi[1] + ball_err_combi[3] + ball_err_combi[5];
                    ball_err_robot[3] = ball_err_combi[2] + ball_err_combi[4] + ball_err_combi[5];

                    int min = 9999;
                    int idx = 0;

                    //다른 로봇의 공 데이터와 비교한 후 가장 다른로봇의 공 데이터와 오차가 적은 로봇의 공 데이터 선택
                    for(int i = 0 ; i < 4 ; i++)
                    {
                        if(ball_err_robot[i] < min){ball_err_robot[i] = min; idx = i;}
                    }
                    ...
                    //현재 로봇에 비전데이터가 존재 할 경우 확정된 공 데이터에서 상대좌표 계산으로 로봇의 위치 재배치 - 현재 이 로봇의 로컬이 틀어졌기 때문
                    if(visionMSG.Ball_2d_X != 0 || visionMSG.Ball_2d_Y != 0)
                    {
                        robot0.x = ball.x - ((-1)*(visionMSG.Ball_2d_X / 10 * cos((robot0.z) * M_PI / 180) + visionMSG.Ball_2d_Y / 10 * sin((robot0.z) * M_PI / 180)));
                        robot0.y = ball.y - (visionMSG.Ball_2d_X / 10 * sin((robot0.z) * M_PI / 180) - visionMSG.Ball_2d_Y / 10 * cos((robot0.z) * M_PI / 180));
                    }
            ...
            //공의 데이터가 1개 존재 할 시
        {
            if(robot_num != -1 && ball_err_ != 0)
                //공의 데이터가 1개 존재 할 시 2
            {
                if(ball_err[robot_num] > ballThreshold)
                    //공 데이터의 오차가 임계값을 넘어갈 경우
                {
                    //공과 가장 가까운 로봇의 데이터 선택
                    ...
                    case 3:{
                        if(min_d > ball.d){min_d = ball.d; min_i = 4;}
                        if(min_i == 4)
                        {
                            ball.x = ball.x;
                            ball.y = ball.y;
...
void robocup_localization24_obstacle::MainWindow::setting()
{
    ...
    if(setting_flag == 0) //setting_flag가 0이면 입출력스트림을 통해 데이터 로드
    {
        ...
        param = "/home/robit/catkin_ws/src/robocup_localization24_obstacle/resources/param/PARAM1.txt";
#endif
#ifdef ROBIT_HUMANOID_ROBOT_NUMBER_2
        param = "/home/robit/catkin_ws/src/robocup_localization24_obstacle/resources/param/PARAM2.txt";
#endif
#ifdef ROBIT_HUMANOID_ROBOT_NUMBER_3
        param = "/home/robit/catkin_ws/src/robocup_localization24_obstacle/resources/param/PARAM3.txt";
#endif
#ifdef ROBIT_HUMANOID_ROBOT_NUMBER_4
        param = "/home/robit/catkin_ws/src/robocup_localization24_obstacle/resources/param/PARAM4.txt";
#endif
#ifdef ROBIT_HUMANOID_ROBOT_NUMBER_5
        param = "/home/robit/catkin_ws/src/robocup_localization24_obstacle/resources/param/PARAM5.txt";
#endif
#ifdef ROBIT_HUMANOID_ROBOT_NUMBER_6
        param = "/home/robit/catkin_ws/src/robocup_localization24_obstacle/resources/param/PARAM6.txt";
#endif
...
void robocup_localization24_obstacle::MainWindow::publish_msg()
{
...
void robocup_localization24_obstacle::MainWindow::Print_Screen() // ui 출력
{
    ...
    if(obs_sector != 1 && obs_sector != 2 && obs_sector != 3 && obs_sector != 4)
    {
    ...
    //    cout << "sector         " << obs_sector << endl;
    //    cout << "obs1           " << obs_obs1 << endl;
    //    cout << "obs2           " << obs_obs2 << endl;
        ...
        if(obs_sector == 1)
        {
            if(obs_obs1 != -1)
            {
                ...
                if(obs_obs1 == 1) //left
                {
                    ...
                    scene->addLine(cvt_Print_xy(robot1.x), cvt_Print_xy(robot1.y), cvt_Print_xy(robot1.x) + sin((-1)*0 * DEG2RAD) * 10, cvt_Print_xy(robot1.y) - cos(0 * DEG2RAD) * 10, blackPen3);
                }
                else if(obs_obs1 == 3) //center
                {
                    robot1.x = 625;
                    robot1.y = 540;
            ...
            if(obs_obs2 != -1)
            {
                ...
                if(obs_obs2 == 1) //left
                {
                    ...
                    scene->addLine(cvt_Print_xy(robot2.x), cvt_Print_xy(robot2.y), cvt_Print_xy(robot2.x) + sin((-1)*0 * DEG2RAD) * 10, cvt_Print_xy(robot2.y) - cos(0 * DEG2RAD) * 10, blackPen3);
                }
                else if(obs_obs2 == 3) //center
                {
                    robot2.x = 625;
                    robot2.y = 540;
                    QPolygonF robot2_poly = create_Print_robot(robot2);
...
void robocup_localization24_obstacle::MainWindow::mouseReleaseEvent(QMouseEvent * e)
{
...
void robocup_localization24_obstacle::MainWindow::mouseMoveEvent(QMouseEvent * e)
{
    //PRE CONDITION : mouseEvent, set_robot_flag, set_ball_flag
    //POST CONDITION : robot0.n, ball.n master_target_n,
...
QPolygonF robocup_localization24_obstacle::MainWindow::create_Print_robot(ROBOT robot)
{
...
int robocup_localization24_obstacle::MainWindow::cvt_Print_xy(float target)
{
...
void robocup_localization24_obstacle::QNode::visionfeatureCallback(const msg_generate::robocup_vision23_feature::ConstPtr &msg)
{
    ...
    if(Likelihood.vision_point_vect.size() > 100) //비전에서 충분한 양의 특징점 데이터를 찾을 시 실행
    {
        ...
        for(int i = 0; i < PARTICLE_NUM; i++)
        {
            ...
            //            cv::imshow("local", Likelihood.localpattern);
            //            cv::imshow("feature", Likelihood.featurepattern);
...
void robocup_localization24_obstacle::QNode::technicalobstaclelocalCallback(const msg_generate::obsnavi::ConstPtr &msg)
{
    if(obs_sector == 1 || obs_sector == 2 || obs_sector == 3 || obs_sector == 4)
    {
//        cout << "0" << endl;
        obsnavi.sector = obs_sector;
        obsnavi.way1 = obs_obs1;
        obsnavi.way2 = obs_obs2;
        if(obsnavi.vision_point_vect.size() > 30) //비전에서 충분한 양의 특징점 데이터를 찾을 시 실행
        {
//            cout << "1" << endl;
            if(vision_data_size / vision_data_cnt <= 1){for(int i = 0; i < PARTICLE_NUM; i++){pt[i].random_point(robot0.x, robot0.y, 20);}}
            for(int i = 0; i < PARTICLE_NUM; i++)
            {
                measuremen_obs.NUM = i;
                measuremen_obs.WEIGHT = obsnavi.sence(pt[i].x, pt[i].y, robot0.x, robot0.y, obsnavi.vision_point_vect);

//                double dis = sqrt(pow(pt[i].x -robot0.x, 2) + pow(pt[i].y -robot0.y, 2));
//                dis /= particle_range;
//                if(dis < 1){dis = 1;}
//                measurement.WEIGHT /= dis;
                particle_weight_obs.push_back(measuremen_obs);

            }
            sort(particle_weight_obs.begin(), particle_weight_obs.end(), sort_return);

            cout << "obstacle matching      " << particle_weight_obs[0].WEIGHT <<endl;
            cout << "x : " << pt[particle_weight_obs[0].NUM].x << endl;
            cout << "y : " << pt[particle_weight_obs[0].NUM].y << endl;

            if(abs(robot0.x - pt[particle_weight_obs[0].NUM].x < 100) && abs(robot0.y - pt[particle_weight_obs[0].NUM].y < 100))
            {
                robot0.x = pt[particle_weight_obs[0].NUM].x;
                robot0.y = pt[particle_weight_obs[0].NUM].y;
            }


//            for(int i = 0; i < PARTICLE_NUM; i++){pt[i].random_point(robot0.x, robot0.y, particle_range);}

            particle_weight_obs.clear();
            obsnavi.vision_point_vect.clear();
            ...
            if(obs_sector == 1)
            {
                for(int i = 0 ; i < 3 ; i++)
                {
                    obsnavi.vision_point.DISTANCE = 1;
                    obsnavi.vision_point.POINT_VEC_X = obsnavi.obs_point_x[i] - robot0.x;
                    obsnavi.vision_point.POINT_VEC_Y = obsnavi.obs_point_y[0] - robot0.y;
                    obsnavi.vision_point.STD_X = robot0.x;
                    obsnavi.vision_point.STD_Y = robot0.y;
                    obsnavi.vision_point_vect.push_back(obsnavi.vision_point);
                }
            }
            else if(obs_sector == 2)
            {
                for(int i = 0 ; i < 3 ; i++)
                {
                    obsnavi.vision_point.DISTANCE = 1;
                    obsnavi.vision_point.POINT_VEC_X = obsnavi.obs_point_x[i] - robot0.x;
                    obsnavi.vision_point.POINT_VEC_Y = obsnavi.obs_point_y[1] - robot0.y;
                    obsnavi.vision_point.STD_X = robot0.x;
                    obsnavi.vision_point.STD_Y = robot0.y;
                    obsnavi.vision_point_vect.push_back(obsnavi.vision_point);
                }
            ...
            else if(obs_sector == 3)
            {
                for(int i = 0 ; i < 3 ; i++)
                {
                    obsnavi.vision_point.DISTANCE = 1;
                    obsnavi.vision_point.POINT_VEC_X = obsnavi.obs_point_x[i] - robot0.x;
                    obsnavi.vision_point.POINT_VEC_Y = obsnavi.obs_point_y[2] - robot0.y;
                    obsnavi.vision_point.STD_X = robot0.x;
                    obsnavi.vision_point.STD_Y = robot0.y;
                    obsnavi.vision_point_vect.push_back(obsnavi.vision_point);
                }
...
//            cout << "3" << endl;
            vision_data_cnt += 1;
            vision_data_size += msg->DISTANCE.size();
            obsnavi.vision_point.DISTANCE = msg->DISTANCE[i];
            obsnavi.vision_point.POINT_VEC_X = (-1)*(msg->POINT_VEC_X[i] / 10 * cos((robot0.z) * M_PI / 180) + msg->POINT_VEC_Y[i] / 10 * sin((robot0.z) * M_PI / 180));
            obsnavi.vision_point.POINT_VEC_Y = (msg->POINT_VEC_X[i] / 10 * sin((robot0.z) * M_PI / 180) - msg->POINT_VEC_Y[i] / 10 * cos((robot0.z) * M_PI / 180));
            obsnavi.vision_point.STD_X = robot0.x;
            obsnavi.vision_point.STD_Y = robot0.y;
            obsnavi.vision_point_vect.push_back(obsnavi.vision_point);
...
void robocup_localization24_obstacle::QNode::imuCallback(const msg_generate::imu_msg::ConstPtr &msg)
{
...
void robocup_localization24_obstacle::QNode::visionCallback(const msg_generate::robocup_vision23::ConstPtr &msg)
{
    //PRE CONDITION : msg(vision)
...
void robocup_localization24_obstacle::QNode::coordinateCallback(const msg_generate::ikcoordinate_msg::ConstPtr &msg)
{
...
void robocup_localization24_obstacle::QNode::gamecontrolCallback(const msg_generate::game_control_data::ConstPtr &msg)
{
...
void robocup_localization24_obstacle::QNode::udpCallback(const msg_generate::localv2_msg::ConstPtr &msg)
{
...
void robocup_localization24_obstacle::MainWindow::distinct()
{
    //PRE CONDITION : robotX.n, obstacleX.n
    //POST CONDITION : obschk
    //purpose : 카메라에 보이는 물체와 udp로 받아온 아군 로봇의 위치를 계산해 물체가 아군 로봇인지 판별

    int linkThreshold = 15;
    if(robot1.x != 0 && robot1.y != 0)
        //아군 로봇에 대한 데이터가 통신될 경우 실행
    {
        double err[4] = {-1, }; //obstacleX에 대한 오차 값을 저장할 변수 선언

        //obstacle이 보일 시 오차 값 계산, 안보이는 경우 오차 값을 -1로 비활성화
        if(obstacle0.x == 0 && obstacle0.y ==0){err[0] = -1;}
        else{
            double err_x = abs(robot1.x - obstacle0.x);
            double err_y = abs(robot1.y - obstacle0.y);

            err[0] = sqrt(pow(err_x, 2) + pow(err_y, 2));
        }

        if(obstacle1.x == 0 && obstacle1.y ==0){err[1] = -1;}
        else{
            double err_x = abs(robot1.x - obstacle1.x);
            double err_y = abs(robot1.y - obstacle1.y);

            err[1] = sqrt(pow(err_x, 2) + pow(err_y, 2));
        }

        if(obstacle2.x == 0 && obstacle2.y ==0){err[2] = -1;}
        else{
            double err_x = abs(robot1.x - obstacle2.x);
            double err_y = abs(robot1.y - obstacle2.y);

            err[2] = sqrt(pow(err_x, 2) + pow(err_y, 2));
        }

        if(obstacle3.x == 0 && obstacle3.y ==0){err[3] = -1;}
        else{
            double err_x = abs(robot1.x - obstacle3.x);
            double err_y = abs(robot1.y - obstacle3.y);

            err[3] = sqrt(pow(err_x, 2) + pow(err_y, 2));
        }

        //계산된 오차값 확인 후 가장 오차값이 작고 임계점(linkThreshold)를 넘지 않을 경우 해당 데이터 저장
        int min = 99999;
        int idx = -1;
        for(int i = 0 ; i < 4 ; i++)
        {
            if(err[i] == -1){continue;}
            if(err[i] < min && err[i] < linkThreshold){idx = i; min = err[i];}
        }

        //계산된 데이터 존재 시 계산된 데이터에 해당되는 obstacle을 아군 로봇으로 간주
        if(min < 99999)
        {
            obschk[idx] = true;
            if(idx == 0){obstacle0.nocnt = 20;obstacle0.x = 0;obstacle0.y = 0;}
            else if(idx == 1){obstacle1.nocnt = 20;obstacle1.x = 0;obstacle1.y = 0;}
            else if(idx == 2){obstacle2.nocnt = 20;obstacle2.x = 0;obstacle2.y = 0;}
            else if(idx == 3){obstacle3.nocnt = 20;obstacle3.x = 0;obstacle3.y = 0;}
            cout << "linking robot1 with obstacle"<< idx << " err : " << min <<endl;
        }
    }
    ...
    if(robot2.x != 0 && robot2.y != 0)
    {
        ...
        if(obstacle0.x == 0 && obstacle0.y ==0){err[0] = -1;}
            ...
            double err_x = abs(robot2.x - obstacle0.x);
            double err_y = abs(robot2.y - obstacle0.y);
        ...
        if(obstacle1.x == 0 && obstacle1.y ==0){err[1] = -1;}
            ...
            double err_x = abs(robot2.x - obstacle1.x);
            double err_y = abs(robot2.y - obstacle1.y);
        ...
        if(obstacle2.x == 0 && obstacle2.y ==0){err[2] = -1;}
            ...
            double err_x = abs(robot2.x - obstacle2.x);
            double err_y = abs(robot2.y - obstacle2.y);
        ...
        if(obstacle3.x == 0 && obstacle3.y ==0){err[3] = -1;}
            ...
            double err_x = abs(robot2.x - obstacle3.x);
            double err_y = abs(robot2.y - obstacle3.y);
        ...
        if(min < 99999)
        {
            ...
            cout << "linking robot2 with obstacle"<< idx << " err : " << min <<endl;
        }
    }
    ...
    if(robot3.x != 0 && robot3.y != 0)
    {
        ...
        if(obstacle0.x == 0 && obstacle0.y ==0){err[0] = -1;}
            ...
            double err_x = abs(robot3.x - obstacle0.x);
            double err_y = abs(robot3.y - obstacle0.y);
        ...
        if(obstacle1.x == 0 && obstacle1.y ==0){err[1] = -1;}
            ...
            double err_x = abs(robot3.x - obstacle1.x);
            double err_y = abs(robot3.y - obstacle1.y);
        ...
        if(obstacle2.x == 0 && obstacle2.y ==0){err[2] = -1;}
            ...
            double err_x = abs(robot3.x - obstacle2.x);
            double err_y = abs(robot3.y - obstacle2.y);
        ...
        if(obstacle3.x == 0 && obstacle3.y ==0){err[3] = -1;}
            ...
            double err_x = abs(robot3.x - obstacle3.x);
            double err_y = abs(robot3.y - obstacle3.y);
        ...
        if(min < 99999)
        {
            ...
            cout << "linking robot3 with obstacle"<< idx << " err : " << min <<endl;
        }
    }
    ...
    if(robot4.x != 0 && robot4.y != 0)
    {
        ...
        if(obstacle0.x == 0 && obstacle0.y ==0){err[0] = -1;}
            ...
            double err_x = abs(robot4.x - obstacle0.x);
            double err_y = abs(robot4.y - obstacle0.y);
        ...
        if(obstacle1.x == 0 && obstacle1.y ==0){err[1] = -1;}
            ...
            double err_x = abs(robot4.x - obstacle1.x);
            double err_y = abs(robot4.y - obstacle1.y);
        ...
        if(obstacle2.x == 0 && obstacle2.y ==0){err[2] = -1;}
            ...
            double err_x = abs(robot4.x - obstacle2.x);
            double err_y = abs(robot4.y - obstacle2.y);
        ...
        if(obstacle3.x == 0 && obstacle3.y ==0){err[3] = -1;}
            ...
            double err_x = abs(robot4.x - obstacle3.x);
            double err_y = abs(robot4.y - obstacle3.y);
        ...
        if(min < 99999)
        {
            ...
            cout << "linking robot4 with obstacle"<< idx << " err : " << min <<endl;
        }
    }

}

void robocup_localization24_obstacle::QNode::masterCallback(const msg_generate::master2localization23::ConstPtr &msg)
{
...
void robocup_localization24_obstacle::QNode::technicalobstalceCallback(const msg_generate::master2localization23_technical_obstacle::ConstPtr &msg)
{
    obs_sector = msg->sector;
    obs_obs1 = msg->obs_1;
    obs_obs2 = msg->obs_2;
}

void robocup_localization24_obstacle::MainWindow::on_btn_free_set_clicked()
{
    //PRE CONDITION : UI
...
void robocup_localization24_obstacle::MainWindow::on_btn_objects_save_clicked()
{
...
void robocup_localization24_obstacle::MainWindow::on_btn_set_1_clicked()
{
    //PRE CONDITION : UI
...
void robocup_localization24_obstacle::MainWindow::on_btn_set_2_clicked()
{
    //PRE CONDITION : UI
    //POST CONDITION : robot0.N, master_target_x
    //purpose : get in시 위치 매크로
...
void robocup_localization24_obstacle::MainWindow::on_btn_set_3_clicked()
{
    //PRE CONDITION : UI
    //POST CONDITION : robot0.N, master_target_x
    //purpose : get in시 위치 매크로
...
void robocup_localization24_obstacle::MainWindow::on_btn_set_4_clicked()
{
    //PRE CONDITION : UI
    //POST CONDITION : robot0.N, master_target_x
    //purpose : get in시 위치 매크로
...
void robocup_localization24_obstacle::MainWindow::on_btn_set_5_clicked()
{
    //PRE CONDITION : UI
    //POST CONDITION : robot0.N, master_target_x
    //purpose : get in시 위치 매크로
...
void robocup_localization24_obstacle::MainWindow::on_btn_set_6_clicked()
{
    //PRE CONDITION : UI
    //POST CONDITION : robot0.N, master_target_x
    //purpose : get in시 위치 매크로
...
void robocup_localization24_obstacle::MainWindow::on_btn_set_auto_clicked()
{
    //PRE CONDITION : UI
    //POST CONDITION : robot0.N, master_target_x
...
void robocup_localization24_obstacle::MainWindow::on_btn_ball_set_clicked()
{
    //PRE CONDITION : UI
...
void robocup_localization24_obstacle::MainWindow::on_btn_ball_del_clicked()
{
    //PRE CONDITION : UI
...
void robocup_localization24_obstacle::MainWindow::on_btn_test_clicked()
{
    robot1.x = 0;
    robot1.y = 0;
    robot1.z = 0;
