    ...
    double sence(int PT_X, int PT_Y, int NOW_X, int NOW_Y, vector<VISION_POINT> &vect)
    {
            ...
            //비전에서 포착한 포인트 수 많큼 연산
        {
            ...
            for(int j = 0; j < 27; j++)
            {
                ...
                if(min_dis > dis){min_dis = dis; pattern[j].X = ptx; pattern[j].Y = pty; pattern[j].Num = j;}
            }
        ...
        //        localpattern = Mat::zeros(2*CIRCLE_R, 2*CIRCLE_R, CV_8UC1);
        //        featurepattern = Mat::zeros(2*CIRCLE_R, 2*CIRCLE_R, CV_8UC1);

        //        Point relative_point;

        //        if(CIRCLE_CENTER.x > CIRCLE_R){relative_point.x =  CIRCLE_CENTER.x -  CIRCLE_R;}
        //        else{relative_point.x = 0;}

        //        if(CIRCLE_CENTER.y > CIRCLE_R){relative_point.y =  CIRCLE_CENTER.y -  CIRCLE_R;}
        //        else{relative_point.y = 0;}
        ...
        //        for(int i = 0 ; i < 27 ; i++)
        //        {
        //            if(Local_point_on_off[i] == 0){continue;}
        //            for(int j = 0 ; j < 27 ; j++)
        //            {
        //                if(Local_point_on_off[j] == 0 || i == j){continue;}
        //                cv::line(localpattern, Point(Local_point_x[i] - relative_point.x, Local_point_y[i] - relative_point.y), Point(Local_point_x[j] - relative_point.x, Local_point_y[j] - relative_point.y), Scalar(255), 10);
        //                recheck_cnt++;
        //            }
        //        }
        ...
        //        for(int i = 0 ; i < 27 ; i++)
        //        {
        //            if(pattern[i].Num == 0){continue;}
        //            for(int j = 0 ; j < 27 ; j++)
        //            {
        //                if(pattern[j].Num == 0 || i == j){continue;}
        //                cv::line(featurepattern, Point(pattern[i].X - relative_point.x, pattern[i].Y - relative_point.y), Point(pattern[j].X - relative_point.x, pattern[j].Y - relative_point.y), Scalar(255), 10);
        //            }
        //        }

        //        for(int i = 0 ; i < 4*CIRCLE_R*CIRCLE_R ; i++)
        //        {
        //            if(localpattern.data[i] & featurepattern.data[i] != 0)
        //            {
        //                pattern_cnt++;
        ...
        //        if(recheck_cnt < 2){return weight;}

        //        if(pattern_cnt < 4*CIRCLE_R*CIRCLE_R/100)
        //        {
        ...
        //        else
        //        {
        //            cout << "??????????????????????     return weight       ??????????????????????" << endl;
        //            return weight;
        //        }

        //        vector<int> dist_vect_local;
        //        vector<int> dist_vect_feature;
        ...
        //            {
        //                if(Local_point_on_off[j] == 0 || i == j){continue;}
        //                int dist = sqrt(pow(Local_point_x[i] - Local_point_x[j], 2) + pow(Local_point_y[i] + Local_point_y[j], 2));
        //                dist_vect_local.push_back(dist);
        //                pattern_cnt++;
        //            }
        //        }
        ...
        //            {
        //                if(pattern[j].Num == 0 || i == j){continue;}
        //                int dist = sqrt(pow(pattern[i].X - pattern[j].X, 2) + pow(pattern[i].Y + pattern[j].Y, 2));
        //                dist_vect_feature.push_back(dist);
        //            }
        //        }

        //        sort(dist_vect_local.rbegin(), dist_vect_local.rend());
        //        sort(dist_vect_feature.rbegin(), dist_vect_feature.rend());
...
public:
    ...
    int obs_point_x[3] = {587, 625, 663};
    int obs_point_y[3] = {540, 390, 240};
    ...
    double sence(int PT_X, int PT_Y, int NOW_X, int NOW_Y, vector<VISION_POINT> &vect)
    {
        ...
        for(int i = 0; i < vect.size(); i++)
        {
            ...
            if(sector == 1)
            {
                if(way2 != -1)
                {
                    int dis1 = 99999999, dis2 = 99999999;

                    if(way1 == 1)
                    {
                        if(way2 == 2)
                        {
                            dis1 = sqrt(pow(obs_point_x[0] - ptx, 2) + pow(obs_point_y[0] - pty, 2));
                            dis2 = sqrt(pow(obs_point_x[2] - ptx, 2) + pow(obs_point_y[0] - pty, 2));
                        ...
                        {
                            dis1 = sqrt(pow(obs_point_x[0] - ptx, 2) + pow(obs_point_y[0] - pty, 2));
                            dis2 = sqrt(pow(obs_point_x[1] - ptx, 2) + pow(obs_point_y[0] - pty, 2));
                    ...
                    else if(way1 == 2)
                    {
                        if(way2 == 2)
                        {
                            dis1 = sqrt(pow(obs_point_x[1] - ptx, 2) + pow(obs_point_y[0] - pty, 2));
                            dis2 = sqrt(pow(obs_point_x[2] - ptx, 2) + pow(obs_point_y[0] - pty, 2));
                ...
                else
                {
                    if(way1 == 1)
                    {
                        dis = sqrt(pow(obs_point_x[0] - ptx, 2) + pow(obs_point_y[0] - pty, 2));
                    }
                    else if(way1 == 2)
                    {
                        dis = sqrt(pow(obs_point_x[1] - ptx, 2) + pow(obs_point_y[0] - pty, 2));
                    }
                    else if(way1 == 3)
                    {
                        dis = sqrt(pow(obs_point_x[2] - ptx, 2) + pow(obs_point_y[0] - pty, 2));
                    }
                }
            }
            else if(sector == 2)
            {
                if(way2 != -1)
                {
                    ...
                    if(way1 == 1)
                    {
                        if(way2 == 2)
                        {
                            dis1 = sqrt(pow(obs_point_x[0] - ptx, 2) + pow(obs_point_y[1] - pty, 2));
                            dis2 = sqrt(pow(obs_point_x[2] - ptx, 2) + pow(obs_point_y[1] - pty, 2));
                        ...
                        {
                            dis1 = sqrt(pow(obs_point_x[0] - ptx, 2) + pow(obs_point_y[1] - pty, 2));
                            dis2 = sqrt(pow(obs_point_x[1] - ptx, 2) + pow(obs_point_y[1] - pty, 2));
                    ...
                    else if(way1 == 2)
                    {
                        if(way2 == 2)
                        {
                            dis1 = sqrt(pow(obs_point_x[1] - ptx, 2) + pow(obs_point_y[1] - pty, 2));
                            dis2 = sqrt(pow(obs_point_x[2] - ptx, 2) + pow(obs_point_y[1] - pty, 2));
                ...
                else
                {
                    if(way1 == 1)
                    {
                        dis = sqrt(pow(obs_point_x[0] - ptx, 2) + pow(obs_point_y[1] - pty, 2));
                    }
                    else if(way1 == 2)
                    {
                        dis = sqrt(pow(obs_point_x[1] - ptx, 2) + pow(obs_point_y[1] - pty, 2));
                    }
                    else if(way1 == 3)
                    {
                        dis = sqrt(pow(obs_point_x[2] - ptx, 2) + pow(obs_point_y[1] - pty, 2));
                    }
                }
            }
            else if(sector == 3)
            {
                if(way2 != -1)
                {
                    ...
                    if(way1 == 1)
                    {
                        if(way2 == 2)
                        {
                            dis1 = sqrt(pow(obs_point_x[0] - ptx, 2) + pow(obs_point_y[2] - pty, 2));
                            dis2 = sqrt(pow(obs_point_x[2] - ptx, 2) + pow(obs_point_y[2] - pty, 2));
                        ...
                        {
                            dis1 = sqrt(pow(obs_point_x[0] - ptx, 2) + pow(obs_point_y[2] - pty, 2));
                            dis2 = sqrt(pow(obs_point_x[1] - ptx, 2) + pow(obs_point_y[2] - pty, 2));
                    ...
                    else if(way1 == 2)
                    {
                        if(way2 == 2)
                        {
                            dis1 = sqrt(pow(obs_point_x[1] - ptx, 2) + pow(obs_point_y[2] - pty, 2));
                            dis2 = sqrt(pow(obs_point_x[2] - ptx, 2) + pow(obs_point_y[2] - pty, 2));
                ...
                else
                {
                    if(way1 == 1)
                    {
                        dis = sqrt(pow(obs_point_x[0] - ptx, 2) + pow(obs_point_y[2] - pty, 2));
                    }
                    else if(way1 == 2)
                    {
                        dis = sqrt(pow(obs_point_x[1] - ptx, 2) + pow(obs_point_y[2] - pty, 2));
                    }
                    else if(way1 == 3)
                    {
                        dis = sqrt(pow(obs_point_x[2] - ptx, 2) + pow(obs_point_y[2] - pty, 2));
                    }
                }
            }
            ...
            //                if(sector == 1)
            //                {
            //                    dis = sqrt(pow(obs_point_x[j] - ptx, 2) + pow(obs_point_y[0] - pty, 2));
            //                }
            //                else if(sector == 2)
            //                {
            //                    dis = sqrt(pow(obs_point_x[j] - ptx, 2) + pow(obs_point_y[1] - pty, 2));
            //                }
            //                else if(sector == 3)
            //                {
            //                    dis = sqrt(pow(obs_point_x[j] - ptx, 2) + pow(obs_point_y[2] - pty, 2));
            //                }
            ...
            weight += (1000 / min_dis) * abs(1 - vect[i].DISTANCE/1000000);
