...
{
    //PRE CONDITION : Mat(input, output), Scalar(min, max),
    //POST CONDITION : output.data
    //PURPOSE : input이미지의 픽셀이 범위안에 들어올 시 255저장, 안 들어올 시 0 저장 - inrange bgr2grayscale, cv네임스페이스의 inrange랑 같은 기능

    register const unsigned int IMGSIZE=input.size().area(); //IMGSIZE에 input 이미지의 크기 저장
    if(output.empty()==true)output=Mat::zeros(input.size(),CV_8UC1); //output 이미지가 공백이면 input이미지를 모두 0 으로 초기화

    //min과 max의 Scalar값 저장
    register const unsigned char A=min[0],B=min[1],C=min[2]
                                ,D=max[0],E=max[1],F=max[2];

    //input과 output의 이미지 데이터 포인터 생성
     ...
     //이미지 크기만큼 반복
    {
        ...
        index=i*3; //index값에 i * 3을 하는 이유는 3채널 이미지의 경우 픽셀 하나당 배열 3개를 차지하기 때문 (bgr)
        if((data[index]>=A)&&(data[index]<=D))
            if((data[++index]>=B)&&(data[index]<=E))
                if((data[++index]>=C)&&(data[index]<=F))
                {
                    //BGR값이 MIN - MAX 범위 안에 존재할 시 해당 픽셀에 해당되는 outputdata에 255저장
                    outputdata[i]=255;
                    continue;
                }
        outputdata[i]=0; //해당되지 않을경우 0 저장
    }
}

void RV::rgb2hsv(const Mat& data_rgb,Mat& data_hsv)
{
    //PRE CONDITION : Mat(data_rgb)
    //POST CONDITION : Mat(data_hsv)
    //PURPOSE : rgb2hsv - inrange bgr2hsv, cv네임스페이스의 inrange랑 같은 기능

    if(data_hsv.empty()==true)data_hsv=Mat::zeros(data_rgb.size(),CV_8UC3); //data_hsv 이미지가 공백이면 data_rgb 이미지를 모두 0 으로 초기화

    //data_rgb data_hsv 이미지 데이터 포인터 생성
    unsigned char* data=       (unsigned char*)data_rgb.data;
    unsigned char* outputdata= (unsigned char*)data_hsv.data;

    //IMGSIZE에 data_rgb 이미지의 크기 저장 및 rgb2hsv변환을 위한 파라미터 선언
...
{
    //PRE CONDITION : Mat(img), Point(PtA, PtB), Scalar(color), LineThresHold
    //POST CONDITION : Line image(선 그려짐)
    //PURPOSE : 선그리기

    unsigned char ucColor=color[0];
    unsigned char* imgData=img.data;

    //PtB와 PtA의 거리 차이 및 절대값, 부호 변수 선언
...
{
    //PRE CONDITION : Mat(_camMat, _dist_Coef), vector(camPos, tilt, objectPoints, imagePoints)
    //POST CONDITION : tilt, camPos,
    //PURPOSE : 영상데이터를 통해 카메라의 위치 및 자세 측정

    Mat distCoeffs(4, 1, CV_64FC1, (double *)_distCoef.data); //기존 행렬 데이터를 사용하여 Mat객체 생성, _distCoef의 데이터가 변경 시 해당 Mat 객체의 데이터도 변경된다.
    Mat rvec, tvec;
...
{
    //PRE CONDITION : tilt, h, focalLength, principalPt, pixelPt
    //POST CONDITION : obj
    //PURPOSE : 영상데이터를 통해 물체의 거리 측정
...
//    cout << "h = " << h << endl;
    ...
    ObjectPos obj; //ObjectPos 객체 생성

    cv::Point2d normPt; //point 객체 생성
...
{
    //PRE CONDITION : contours, hierarchy, contourIdx, dst
    //POST CONDITION : dst에 contour적용
    //PURPOSE : 컨투어 데이터를 통해 이미지에 컨투어 적용

    if(dst.channels() != 1)
    {
        //저장될 이미지가 8비트 1채널이 아닐 경우 강제 종료
        cout << "In function fillInContours: dst image must be 8-bit, 1 1channel." << endl;
        exit(1);
    }

    Mat drawing = Mat::zeros(dst.size(), CV_8UC1); //dst의 사이즈와 8비트1채널을 가지고 흑백인 이미지 생성
    cv::drawContours(dst, contours, contourIdx, Scalar(74), 2, LINE_8, hierarchy, 0, Point()); //dst에 컨투어 그리기
