...
#include <QMessageBox>
#include <iostream>
#include "../include/c_robocup_vision23_yolov4/main_window.hpp"
...
//For Pan_Tilt
PAN_TILT pan_tilt = PAN_TILT(2000,2000);
int scan_value = 0;
...
** Functions
*****************************************************************************/
void c_robocup_vision23_yolov4::MainWindow::publish_vision_msg()
{
    ...
    visionMsg.Robot_2d_X = robot_absx;
    visionMsg.Robot_2d_Y = robot_absy;
    ...
    //sujong
    sprintf(TEXT, "ROBOT_X : %lf", visionMsg.Robot_2d_X);ui.ROBOT_X_value->setText(TEXT);
    sprintf(TEXT, "ROBOT_Y : %lf", visionMsg.Robot_2d_Y);ui.ROBOT_X_value_2->setText(TEXT);
...
void c_robocup_vision23_yolov4::MainWindow::publish_localization_msg()
{
...
void c_robocup_vision23_yolov4::MainWindow::filter_100ms()
{
...
void c_robocup_vision23_yolov4::MainWindow::update(const cv::Mat& img, const cv::Mat& info)
{
...
Mat c_robocup_vision23_yolov4::MainWindow::Image_processing(const cv::Mat& img)
{
    data.clear();
    //HSV
    ...
    if(cam_nice_point != 0) //yolo
    {
        ...
        if(cnt_line != 0)
        {
            ...
            if(pts.size()>0)
            {
                undistortPoints(pts, pts, K_M, D_M, Mat(), NEW_K_M);//sujong
            }
        ...
        if(cnt_robot != 0)//robot_vision
        {
            Rect result(0,0,0,0);
            double score = 0;
            for(int i = 0; i < yolov4.vector_yolo.size();i++)
            {
                if((yolov4.vector_yolo[i].NUM == 2) && (yolov4.vector_yolo[i].SCORE > score))
                {
                    result = Rect(yolov4.vector_yolo[i].X, yolov4.vector_yolo[i].Y, yolov4.vector_yolo[i].W, yolov4.vector_yolo[i].H);
                    score = yolov4.vector_yolo[i].SCORE;
                }
    ...
    if(save_img_flag && (ballPos.dist != 0 || robot_absx !=0))//레알 개대충 조건
    {
        save_img_flag = 0;

        char buf[256];
        sprintf(buf, "/home/robit/catkin_ws/src/c_robocup_vision23_yolov4/img/%s%06d.png", str_list,data_num);
        imwrite(buf, img);
        sprintf(buf, "/home/robit/catkin_ws/src/c_robocup_vision23_yolov4/img/%s%06d.txt", str_list,data_num);
        ofstream file(buf);
        if(file.is_open())
        {
...
void c_robocup_vision23_yolov4::MainWindow::setting()
{
...
void c_robocup_vision23_yolov4::MainWindow::save_hsv()
{
    if(setting_flag != -1)
    {
        ofstream Last_Index_Num_OUT("/home/robit/catkin_ws/src/c_robocup_vision23_yolov4/resources/HSV/HSV.txt");
...
void c_robocup_vision23_yolov4::MainWindow::Print_Screen(const cv::Mat& img, const cv::Mat& result_img)
{
    //INIT
...
void c_robocup_vision23_yolov4::QNode::masterCallback(const msg_generate::master_2_robocup_vision23::ConstPtr &msg)
{
    scan_value = msg->Scan_Mode;
    if(scan_value == 4)
    {
...
void c_robocup_vision23_yolov4::MainWindow::on_btn_mode_0_clicked()
{
    scan_value = 0;
}
void c_robocup_vision23_yolov4::MainWindow::on_btn_mode_1_clicked()
{
    scan_value = 2;
}
void c_robocup_vision23_yolov4::MainWindow::on_btn_mode_2_clicked()
{
    scan_value = 3;
}
void c_robocup_vision23_yolov4::MainWindow::on_btn_mode_3_clicked()
{
    save_img_flag = 1;

    if(autosave_flag_1s == 0){autosave_flag_1s = 1;cout<<"autosavestart!!"<<endl;}else{autosave_flag_1s = 0;};//sujung


}
void c_robocup_vision23_yolov4::MainWindow::on_radioori_clicked(bool checked)
{
    HSV_flag = 0;
    setting_flag = 0;
}
void c_robocup_vision23_yolov4::MainWindow::on_radiofield_clicked(bool checked)
{
    HSV_flag = 1;
    setting_flag = 0;
}
void c_robocup_vision23_yolov4::MainWindow::on_radioline_clicked(bool checked)
{
    HSV_flag = 2;
    setting_flag = 0;
}

void c_robocup_vision23_yolov4::MainWindow::on_btn_oper_pan_tilt_clicked(bool checked)
{
