...
#include <sstream>
#include "../include/robocup_vision24_yolov4/qnode.hpp"
...
bool QNode::init() {
    ros::init(init_argc,init_argv,"robocup_vision24_yolov4");
    if ( ! ros::master::check() ) {
        return false;
    }
    ...
    image_sub = it.subscribeCamera("camera/image_raw", 100, &robocup_vision24_yolov4::QNode::imageCallBack, this);

    //PAN_TILT Topic
    motor_dxl_Publisher = n.advertise<serial_mcu::Motor_msg>("Dynamixel", 100);

    //Vision Topic
    visionPub = n.advertise<msg_generate::robocup_vision24>("robocup_vision24", 100);
    vision_feature_Pub = n.advertise<msg_generate::robocup_vision24_feature>("robocup_vision24_feature", 100);

    //Master Topic
    masterSub = n.subscribe("master_2_robocup_vision24", 100, &QNode::masterCallback, this);
