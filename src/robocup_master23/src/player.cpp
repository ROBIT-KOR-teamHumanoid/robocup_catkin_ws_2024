#include "robocup_master23/player.hpp"
...
bool Player::falldownExeption()
{
    ...
    if(isFalldown(55, 40, falldownWhere)) { //20240404
        rst = true;
        walkStop();
        cnt++;
        cout << "cnt: " << cnt <<  endl;
...
void Player::gameStateControl(bool control_on, int state, int position)
{
    if(!control_on){
        return;
    }
    if(position == TECHNICAL)//technical
    {
        switch (state) {
        case STATE_CL_K:
            statePlay();
...
void Player::selectRobotState()
{
    ...
    double shortestDist = calcDistance(myPosition, ballPosition);
    double distCheck = shortestDist;
    for (int robotIndex = 0; robotIndex < MAX_ROBOT_NUM; robotIndex++) {
        if(master->udp[robotIndex].robot_num == 0 || master->udp[robotIndex].robot_num == master->gameControlData.robotNum) {
            continue;
        }

        Point robotPosition(master->udp[robotIndex].local_x, master->udp[robotIndex].local_y);
        double tempDist = calcDistance(robotPosition, ballPosition);
...
int Player::isFalldown(const double pitch_threshold, const double roll_threshold, int& falldownWhere)
{
    ...
    if(roll > roll_threshold) {
        ...
        cout << "imu.pitch:: " << pitch << endl;
        cout << "pitch_threshold:: " << pitch_threshold << endl; //20240408
    } else if(pitch < -pitch_threshold) {
...
void Player::raisetUp(int falldownWhere)
{
    ...
    case FALLDOWN_FRONT:
        //playMotion(1);
        playMotion(MOTION_RASEUP_FRONT); //20240404
        break;
    case FALLDOWN_REAR:
...
void Player::kick3()
{
    ...
    const double obstacle_threshold = 100; //상대 로봇과의 거리 파라미터: 이 값보다 작으면 빠르게 보정한다.
    ...
    static int state;
    const int refDist = 20;//30;//cm  30
    const int distBias = 10;

    Point pointWhereIGo = calcShootPoint(refDist, goalPoint, globalBall);

    if(state < 3) {
        if(calcDistance(globalRobot, globalBall) > refDist + distBias)
        {
            state = 0;
        }
...
//            state = 0;
    ...
    case 3:
    {
        ...
        if(cnt_delayBeforeMotion > 300) { //1s // 500 -> 300 20240408
    ...
    case 4:
        ...
        {
            kick_mode = FRONT_LEFT;//FRONT_LEFT;
            cout << "!!!!OPERATOR LEFT KICK!!!!" << endl;
            //playMotion(3); //3: LEFT 20240502
        }
        else
        {
            kick_mode = FRONT_RIGHT;
            cout << "!!!!OPERATOR RIGHT KICK!!!!" << endl;
            //playMotion(2); //202404029
        }
        state = 5;
...
//    else if(z > 5) z = 5;
    ...
    walkStart(-10 * reg_radiusErr, dir*y, 1.5*z);
...
bool Player::caliToBall(Point goalPoint) //local base
{
    ...
    const int front = 300;//200; // number2: 120 -> 150 //least  116  190
...
//    }
    ...
    PIDControll yawControl(6.0, 0, 0, R_YAW_MAX, 0);
